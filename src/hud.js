...
export default function HUD() {
  ...
  function createShipPlacementBox(
    ...
    const startGameBtn = createStartGameButton(startGameCallBack);
    ...
    for (const [key, value] of Object.entries(shipPlacementDict)) {
      ...
      const container = createContainer(key, draggedData);
      const shipBoxConatiner = createShipBoxContainer();
      const amountLabel = createAmountLabel(key, value);

      //createShipImage(shipBoxConatiner,key,horiontalPlacementInput.checked== "true" ? "h" : "v")
